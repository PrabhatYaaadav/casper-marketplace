...
export class MarketTester {
    ...
    public async ownerOf(token_id: string){
    const ownerOfToken = await this.cep78.getOwnerOf(token_id);
    console.log(`...... ${ownerOfToken} has token ${token_id}`);
    }
    ...
    public async buyListing(
        ...
        console.log(buyerKeys.accountHash.toString())
        console.log(sellerKeys.accountHash.toString())
        const installDeployHash = await this.paymentClient.install(
            this.paymentWasmPath, {
                ...
                is_hash_identifier_mode: is_hash_identifier_mode,
                token_id: token_id,
                amount: parseInt(price),
                target_key: buyerKeys.accountHash(),
                source_key: sellerKeys.accountHash()
            },
        ...
        let ownerOfToken = await this.cep78.getOwnerOf(token_id);
        ...
        ownerOfToken = await this.cep78.getOwnerOf(token_id);
